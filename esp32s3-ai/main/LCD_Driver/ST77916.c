#include "ST77916.h"
#include "../LVGL_Driver/LVGL_Driver.h"  // for disp_drv to signal LVGL flush complete
...
static st77916_vendor_config_t vendor_config = {
  .init_cmds = NULL,            // will be set dynamically if needed
  .init_cmds_size = 0,
  .flags = { .use_qspi_interface = 1 }, // we are using QSPI (4 data lines)
};
...
static bool lcd_color_trans_done_cb(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_io_event_data_t *edata, void *user_ctx) {
  // Signal LVGL that the flush is complete (async mode)
...
int QSPI_Init(void){
  ...
  // Basic required init: Sleep Out (0x11), COLMOD (0x3A=16bpp), Display ON (0x29) in case vendor table skipped
  ...
  // Read register 0x04 for vendor differentiation
  esp_err_t ret;
  int lcd_cmd = 0x04;
  ...
  lcd_cmd &= 0xff;
  lcd_cmd <<= 8;
  lcd_cmd |= LCD_OPCODE_READ_CMD << 24;  // read opcode
  ...
  if (register_data[0] == 0x00 && register_data[1] == 0x7F && register_data[2] == 0x7F && register_data[3] == 0x7F) {
    printf("Vendor-specific initialization for case 1. Using default driver init table.\n");
    ...
    printf("Vendor-specific initialization for case 2. Overriding init table.\n");
  } else {
    printf("Unknown panel ID pattern, proceeding with default init.\n");
  }
