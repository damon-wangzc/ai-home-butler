...
void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    esp_err_t r = esp_lcd_panel_draw_bitmap(panel_handle, offsetx1, offsety1, offsetx2 +1, offsety2 + 1, color_map);
    if (r != ESP_OK) {
        ESP_LOGE("LVGL_FLUSH", "draw_bitmap failed err=%d area=(%d,%d)-(%d,%d)", r, offsetx1, offsety1, offsetx2, offsety2);
        lv_disp_flush_ready(drv); // fail-safe release
    }
    // Do not call lv_disp_flush_ready here in async mode; callback in ST77916.c will signal completion
}
...
void LVGL_Init(void)
{
    ...
    // Single small internal DMA buffer (Option A) to free internal RAM for speech engine
    lv_color_t *buf1 = heap_caps_malloc(LVGL_BUF_LEN * sizeof(lv_color_t), MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    assert(buf1);
    lv_disp_draw_buf_init(&disp_buf, buf1, NULL, LVGL_BUF_LEN);  // second buffer NULL (single buffering)
